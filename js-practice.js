...
function dividedBy2(numbers) {
  const numbersDividedBy2 = [];
  for (let i = 0; i < numbers.length; i++) {
    numbersDividedBy2.push(numbers[i] / 2);
...
function firstLetterOnly(strings) {
  ...
  for (let i = 0; i < strings.length; i++) {
    firstLetterOnly.push(strings[i][0]);
  }
  console.log(firstLetterOnly);
}

firstLetterOnly(["hello", "goodbye"]);

// # 9.  Start with an array of hashes and create a new array of number values from each hash's :age key times 2.
// #     For example, [{name: "Alice", age: 27}, {name: "Blane", age: 16}] becomes [54, 32].

function agesTimes2(peopleObject) {
  const justAgesTimes2 = [];
  for (let i = 0; i < peopleObject.length; i++) {
    justAgesTimes2.push(peopleObject[i].age * 2);
  }
  console.log(justAgesTimes2);
}

agesTimes2([{ name: "Alice", age: 27 }, { name: "Blane", age: 16 }]);
...
function toString(numbers) {
  ...
  for (let i = 0; i < numbers.length; i++) {
    numbersToStrings.push(numbers[i].toString());
  }
